...
public class RobustWebElement implements WebElement {
    ...
    public RobustWebElement(WebElement originalWebElement, RobustWebDriver driver, By by) {
    ...
    private void executeMethodWithRetries(Consumer<WebElement> method) {
        ...
        while (retries < MAX_RETRIES) {
            try {
                WebElementUtils.callMethod(originalWebElement, method);
                return;
    ...
    private <T> T executeMethodWithRetries(Function<WebElement, T> method) {
        ...
        while (retries < MAX_RETRIES) {
            try {
                return WebElementUtils.callMethodWithReturn(originalWebElement, method);
            } catch (StaleElementReferenceException ex) {
                refreshElement();
    ...
    private <U> void executeMethodWithRetriesVoid(BiConsumer<WebElement, U> method, U parameter) {
        ...
        while (retries < MAX_RETRIES) {
            try {
                WebElementUtils.callMethod(originalWebElement, method, parameter);
                return;
            } catch (StaleElementReferenceException ex) {
                refreshElement();
            }
    ...
    private <T, U> T executeMethodWithRetries(BiFunction<WebElement, U, T> method, U parameter) {
        ...
        while (retries < MAX_RETRIES) {
            try {
                return WebElementUtils.callMethodWithReturn(originalWebElement, method, parameter);
            } catch (StaleElementReferenceException ex) {
                refreshElement();
            }
            retries++;
    ...
    private void refreshElement() {
        this.originalWebElement = driver.findElement(by);
    }
